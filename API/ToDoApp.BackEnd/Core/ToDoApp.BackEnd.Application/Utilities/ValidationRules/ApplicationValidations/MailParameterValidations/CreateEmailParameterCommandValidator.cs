...
public sealed class CreateEmailParameterCommandValidator : AbstractValidator<CreateEmailParameterCommand>
{
    public CreateEmailParameterCommandValidator()
    {
        RuleFor(x => x.Email)
      ...
      .EmailAddress().WithMessage("Email must be in a valid email format.");
        ...
        RuleFor(x => x.EmailParameterType)
